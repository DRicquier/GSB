...
namespace GSB
{
    static class Passerelle
    {
        ...
        static public void chargerDonnees()
        {
            ...
            while (curseur.Read()) {
                string idType = curseur["id"].ToString();
                string libelle = curseur["libelle"].ToString();


                TypePraticien unType = new TypePraticien(idType, libelle);
            }

            curseur.Close();

            string sql = "Select id, nom, prenom, rue, codePostal, ville, téléphone, email, idType, idSpecialite from mesPraticiens";
            MySqlCommand cmd = new MySqlCommand(sql, cnx);
            MySqlDataReader curseur = cmd.ExecuteReader();
            while (curseur.Read()) {
                ...
                string codePostal = curseur["codePostale"].ToString();
                string rue = curseur["rue"].ToString();
                string ville = curseur["ville"].ToString();
                string telephone = curseur["téléphone"].ToString();
                string email = curseur["email"].ToString();
                string idType = curseur["idType"].ToString();
                string idSpecialite = curseur.IsDBNull(9) ? null : curseur["idSpecialite"].ToString();

                Praticien praticien = new Praticien(id, nom, prenom, codePostal, rue, ville, telephone, email, idType, idSpecialite)
            }
